package me.killstorm103.Rebug.SoftWares.Spigot.Events;
...
public class EventMenus_Spigot implements Listener 
{
	...
	private boolean ShouldCancelAction (String MenuName, String Type)
	{
		...
		if ((Type.equals("Drag") || Type.equals("Click")) && (MenuName.equalsIgnoreCase("Rebug Debug Item Menu") || MenuName.equalsIgnoreCase("Player Settings") || MenuName.equalsIgnoreCase("Packet Selector") || MenuName.equalsIgnoreCase("Rebug Settings") || MenuName.equalsIgnoreCase("anticheats") || MenuName.equalsIgnoreCase("items") || MenuName.equalsIgnoreCase("Potions") || MenuName.equalsIgnoreCase("Client Information") || MenuName.equalsIgnoreCase("Exploits") || MenuName.equalsIgnoreCase("Vanilla Fly Checks")) )
			return true;
	...
	public void onMenu(InventoryClickEvent e) 
	{
		...
		if (PT.isStringNull(MenuName)) return;
		...
		if (MenuName.equalsIgnoreCase("Rebug Debug Item Menu"))
		{
			if (item != null && meta != null && meta.hasDisplayName())
			{
				if (getCursor != null && getCursor.getType() != Material.AIR && item.getType() == ItemsAndMenusUtils.INSTANCE.getDebugItem().getType())
				{
					Rebug.getINSTANCE().getPT().sendMessage(player, "Displayname: " + (getCursor.hasItemMeta() && getCursor.getItemMeta().hasDisplayName() ? getCursor.getItemMeta().getDisplayName() : "Not Set"));
					Rebug.getINSTANCE().getPT().sendMessage(player, "Material Type: " + (!PT.isStringNull(getCursor.getType().name()) ? getCursor.getType().name() + " (" + getCursor.getType() + ")" : "Null"));
					if (!PT.isServerNewerOrEquals(ServerVersion.V_1_13))
						Rebug.getINSTANCE().getPT().sendMessage(player, "Material ID: " + getCursor.getType().getId());
						
					Rebug.getINSTANCE().getPT().sendMessage(player, "ItemData: " + (getCursor.getData() != null ? getCursor.getData().getData() : "None"));
					if (getCursor.hasItemMeta() && getCursor.getItemMeta().hasEnchants())
					{
						Rebug.getINSTANCE().getPT().sendMessage(player, "Enchants: ");
						final boolean is13 = PT.isServerNewerOrEquals(ServerVersion.V_1_13);
						for (Map.Entry<Enchantment, Integer> map : getCursor.getItemMeta().getEnchants().entrySet())
						{
							if (is13)
								Rebug.getINSTANCE().getPT().sendMessage(player, "Name: " + map.getKey().getName() + " Level: " + map.getValue());
							else
								Rebug.getINSTANCE().getPT().sendMessage(player, "Name: " + map.getKey().getName() + " ID: " + map.getKey().getId() + " Level: " + map.getValue());
						}
					}
				}
				e.setCancelled(true);
		...
		if (user == null)
		{
			player.sendMessage(PT.RebugsUserWasNullErrorMessage("in menu (" + MenuName + ")"));
			return;
		}
		if (MenuName.equalsIgnoreCase("items"))
		{
			...
			if (PT.isInventoryFull(e.getView().getBottomInventory()))
			{
				user.getPlayer().sendMessage("Your Inventory's full make some space!");
				return;
			}
			if (item == null || item.getType() == ItemsAndMenusUtils.INSTANCE.getDeadGlassItem().getType()) return;
		...
		if (MenuName.equalsIgnoreCase("Client Information"))
		{
			...
			e.setCancelled(true);
			if (!PT.isStringNull(ItemName) && ItemName.equalsIgnoreCase("network"))
			{
		...
		if (MenuName.equalsIgnoreCase("AntiCheats"))
		{
			if (e.getView().getTopInventory() == e.getClickedInventory())
			{
				...
				if (item == null || !item.hasItemMeta() || item.getItemMeta().getLore() == null
						|| item.getItemMeta().getLore().size() <= 1 || PT.isStringNull(ItemName) || ItemName.equalsIgnoreCase("info"))
					return;
		...
		if (MenuName.equalsIgnoreCase("Exploits"))
		{
			...
			e.setCancelled(true);
			if (!ItemName.equalsIgnoreCase("User " + user.getPlayer().getName()) && item.getType() != ItemsAndMenusUtils.INSTANCE.getDeadGlassItem().getType())
			{
		...
		if (MenuName.equalsIgnoreCase("Vanilla Fly Checks"))
		{
			...
			if (item != null && ItemName != null && item.getType() != ItemsAndMenusUtils.INSTANCE.getDeadGlassItem().getType())
			{
