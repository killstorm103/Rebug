package me.killstorm103.Rebug.Shared.Command.Commands.Handler;
	...
	public void onCommandPreProcess(PlayerCommandPreprocessEvent e) {
		...
		if (!ShowCommands.OpPlayers.isEmpty()) {
			for (Map.Entry<String, UUID> ops : ShowCommands.OpPlayers.entrySet()) {
				...
				if (op != null && op.isOnline() && op != player
						&& !ShowCommands.OpPlayers.containsValue(player.getUniqueId())) {
		...
		if (command.startsWith("/rules")) {
			...
			for (int i = 0; i < Config.getServerRules().size(); i++) {
				user.getPlayer()
						.sendMessage(ChatColor.BOLD.toString() + ChatColor.DARK_GRAY + "| " + ChatColor.DARK_RED
								+ "SERVER " + ChatColor.RED + "Rules " + ChatColor.DARK_GRAY + ">> " + (i == 0 ? 1 : i)
								+ " " + ChatColor.RED + Config.getServerRules().get(i));
			}
			e.setCancelled(true);
			return;
		}
		if (!Rebug.hasAdminPerms(user)) {
			...
			// TODO Make a blocker for /plugins and /help and display my own version of
			// /help
			if (Flagged) {
				e.setCancelled(true);
		...
		if (!Rebug.getINSTANCE().getConfig().getBoolean("allow-sub-commands"))
			return;
		...
		if (Split.length > 0) {
			...
			if (c != null) {
				...
				Bukkit.dispatchCommand(user.getPlayer(), "rebug " + c.getName() + (c.RemoveSlash() ? " " : "")
						+ (c.RemoveSlash() ? command.replace("/", "") : command.replace(Split[0], "")));
