...
public class EventMenus implements Listener {
	...
	public void onMenu(InventoryClickEvent e) 
	{
		...
		if (Rebug.debug)
		{
			Rebug.Debug(player, "slot= " + slot + " item= " + item + (item.hasItemMeta() && item.getItemMeta().hasDisplayName() ? " ItemName= " + ChatColor.stripColor(item.getItemMeta().getDisplayName())  : "") + " Menu= " + MenuName);
			Rebug.Debug(player, "ClickType= " + e.getClick().name());
		}
		if ((e.getInventory().getType() == InventoryType.ENCHANTING
				|| e.getInventory().getType() == InventoryType.ANVIL) && !player.isOp()
				&& !player.hasPermission("me.killstorm103.rebug.server_owner")
				&& !player.hasPermission("me.killstorm103.rebug.server_admin")) {
			e.setCancelled(true);
			player.sendMessage(Rebug.RebugMessage + "You can't do that you don't have perms!");
			return;
		}
		if (MenuName != null) {
			if (e.getClickedInventory() != e.getWhoClicked().getInventory() && item != null
					&& MenuName.equalsIgnoreCase("Items") && item.getType() == Material.WOOL) {
			...
			if (e.getClickedInventory() != e.getWhoClicked().getInventory() && item != null
					&& (MenuName.equalsIgnoreCase("Items") || MenuName.equalsIgnoreCase("Crashers")
							|| MenuName.equalsIgnoreCase("Exploits"))
		...
		if (MenuName != null)
		{
			...
			if (user == null)
			{
				PTNormal.KickPlayer((Player) e.getView().getPlayer(),
						PTNormal.RebugsUserWasNullErrorMessage("in " + MenuName));
				return;
			}
			if (MenuName != null && (inventoryAction == InventoryAction.HOTBAR_SWAP || inventoryAction == InventoryAction.HOTBAR_MOVE_AND_READD || inventoryAction == InventoryAction.SWAP_WITH_CURSOR || inventoryAction == InventoryAction.PLACE_ALL || 
					inventoryAction == InventoryAction.PLACE_ONE || inventoryAction == InventoryAction.PLACE_SOME || inventoryAction == InventoryAction.MOVE_TO_OTHER_INVENTORY))
			{
			...
			if (item != null && item.getItemMeta() != null) 
			{
				...
				if (MenuName.equalsIgnoreCase("Potions")
						&& e.getClickedInventory() != user.getPlayer().getInventory()) {
					e.setCancelled(true);
				...
				if (MenuName.equalsIgnoreCase("Client Information") && e.getClickedInventory() != user.getPlayer().getInventory())
				{
					e.setCancelled(true);
				...
				if (MenuName.equalsIgnoreCase("Exploits") && e.getClickedInventory() != user.getPlayer().getInventory()
						&& ItemName != null) {
					e.setCancelled(true);
				...
				if (MenuName.equalsIgnoreCase("Rebug Settings") && Rebug.hasAdminPerms(user.getPlayer())
						&& e.getClickedInventory() != user.getPlayer().getInventory()) {
					e.setCancelled(true);
