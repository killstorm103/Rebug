...
public class ItemsAndMenusUtils
{
	...
	{
		if (inventory == null || PTNormal.isStringNull(text) || inventory.getItem(itemslot) == null || !inventory.getItem(itemslot).hasItemMeta()) return;
	...
	public Inventory getItemPickerMenu ()
	{
		...
		if (ItemPickerMenu != null)
		{
			...
			{
				ItemPickerMenu.setItem(Config.getItemMenuSize()-1, Rebug.getINSTANCE().getNMS().getMadeItems("%delete_item%"));
			}
			else
				ItemPickerMenu.setItem(Config.getItemMenuSize() - 1, deadglass);
			
			if (Config.getItemMenuDebugItem())
				ItemPickerMenu.setItem(Config.getItemMenuSize()-2, Rebug.getINSTANCE().getNMS().getMadeItems("%debug_item%"));
			else
