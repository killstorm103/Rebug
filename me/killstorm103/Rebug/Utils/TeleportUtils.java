...
public class TeleportUtils {
	...
	public static Location GenerateCrashLocation(Player player) {
		final Location To = new Location(player.getWorld(), player.getLocation().getX(), player.getLocation().getY(),
				player.getLocation().getZ());
		double y = To.getY();
		while (y > -2.147483648E9D) {
			y -= 1000000;
			To.setY(y);
			Location randomLocation = new Location(player.getWorld(), player.getLocation().getX(), To.getY(),
					player.getLocation().getZ());
