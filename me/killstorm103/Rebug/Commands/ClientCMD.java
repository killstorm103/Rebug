		...
		if (sender instanceof Player)
		{
			...
			if (args.length < 2)
			{
				...
				if (brand == null || commander_user.BrandSetCount < 1 || brand.length() <= 0)
				{
	    			...
	    			{
	    				PTNormal.KickPlayer(commander_user.getPlayer(), Rebug.RebugMessage + "kicked you due to failing to load your client brand!");
	    				return;
	    			}
	    			else if (Config.getClientInfoSetting().equalsIgnoreCase("ban"))
	    			{
	    				PTNormal.BanPlayer(commander_user.getPlayer(), Rebug.RebugMessage + "banned you due to failing to load your client brand!");
	    				return;
	    			}
				}
			...
			if (commander.hasPermission(getPermission() + "_others") || Rebug.hasAdminPerms(commander))
			{
				...
				if (brand == null || user_target.BrandSetCount < 1 || brand.length() <= 0)
				{
	    			...
	    			{
	    				PTNormal.KickPlayer(user_target.getPlayer(), Rebug.RebugMessage + "kicked you due to failing to load your client brand!");
	    				return;
	    			}
	    			if (Config.getClientInfoSetting().equalsIgnoreCase("ban"))
	    			{
	    				PTNormal.BanPlayer(user_target.getPlayer(), Rebug.RebugMessage + "banned you due to failing to load your client brand!");
	    				return;
	    			}
				}
