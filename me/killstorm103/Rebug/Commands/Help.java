		...
		if (args.length == 1) {
			Log(sender,
					ChatColor.BOLD.toString() + ChatColor.DARK_GRAY + "| " + ChatColor.DARK_RED + "REBUG "
							+ ChatColor.GRAY + "v" + Rebug.PluginVersion() + " " + ChatColor.BOLD.toString()
							+ ChatColor.DARK_GRAY + " | " + ChatColor.GRAY + " made by " + ChatColor.YELLOW
							+ Rebug.getAuthor() + " " + ChatColor.BOLD.toString() + ChatColor.DARK_GRAY + "|",
					false);
			Log(sender,
					ChatColor.GRAY + "commands (" + "You have access to "
							+ Rebug.getINSTANCE().getAccessToCommandsNumber(sender) + " / "
							+ Rebug.getINSTANCE().getCommands().size() + ")" + ChatColor.RESET + ":",
					false);
		}
		if (args.length == 3) {
			...
			if (args.length == 1) {
				if (sender instanceof Player) {
					if (commands.HideFromCommandsList(sender)
							&& !((Player) sender).hasPermission(Rebug.AllCommands_Permission)) {
					} else
