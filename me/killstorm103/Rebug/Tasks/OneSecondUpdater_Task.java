			...
			if (user != null)
			{
				...
				if (Bukkit.getOnlinePlayers().size() > 1)
				{
					for (Player target : Bukkit.getOnlinePlayers())
					{
						...
						{
							if (user.HideOnlinePlayers || user.ProximityPlayerHider && user.getPlayer().getLocation().distance(target.getLocation()) <=
							Rebug.getINSTANCE().getConfig().getDouble("proximity-player-hider"))
							{
