...
public class ItemsAndMenusUtils
{
		...
		if (RebugSettingsMenu == null)
		{
			...
			try
			{
				for (Map.Entry<String, String> list : RebugPlugin.getSettingsList().entrySet())
				{
					...
					Material RebugSetting = Material.getMaterial(FileUtils.getMenusConfig().getConfigurationSection("Rebug Settings").getString(path + ".material"));
	...
	public Inventory getAntiCheats ()
	{
		...
		{
			Inventory inventory = PT.createInventory(null, FileUtils.getLoadedAntiCheatsFile().getInt("loaded-anticheats.Inventory-size"), ChatColor.RED + "AntiCheats");
			int size = Config.getLoadedAntiCheats().size(), manualsize = FileUtils.getLoadedAntiCheatsFile().getStringList("manually-added-anticheats.ac-names").size();
