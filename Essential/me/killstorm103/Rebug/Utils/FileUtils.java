	...
	public static void restorePlayer (@NotNull Object object)
	{
		...
		for (Map.Entry<String, String> setting : user.getSettingsList().entrySet())
		{
			...
			case "boolean":
				...
				if (FileUtils.getPlayerSettingsConfig().getString(name + ".permission") != null && !PT.isStringNull(FileUtils.getPlayerSettingsConfig().getString(name + ".permission")) && !PT.hasPermission (user, FileUtils.getPlayerSettingsConfig().getString(name + ".permission")))
				{
					if (isEnabled)
		...
		String def = getLoadedAntiCheatsFile().getString("default-anticheat");
		if (user.getSelectedAntiCheats() < 2) 
		{
			user.setAntiCheat(PT.hasAdminPerms(user) || getLoadedAntiCheatsFile().getBoolean("bypass-permission-enabled")
					...
					: getLoadedAntiCheatsFile().getBoolean("force-default-anticheat") ? user.getAntiCheat()
							: playerConfig.getConfigurationSection("Player Settings").getString("AntiCheat"));
