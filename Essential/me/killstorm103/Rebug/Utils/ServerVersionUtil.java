...
public class ServerVersionUtil 
{
	...
	@Getter @Setter
	private static boolean isBedRockSupported, isServerNewerThan1_8 = true, isSpoofedSoftWare = false, isFolia;
	@Getter @Setter
	...
	public static void CreateINSTANCE (String name)
	{
		...
		String toString = "";
		final int size = FileUtils.getServerConfig().getStringList("server.softwares.force.scan.types").size();
		int detected = 0;
		for (int i = 0; i < size; i ++)
		{
			String ST = FileUtils.getServerConfig().getStringList("server.softwares.force.scan.types").get(i);
			if (isServerType(ST))
			{
				detected ++;
	...
	public static boolean isServerType (@NotNull String type) 
	{
		...
		for (int i = 0; i < size; i ++)
		{
			try
			{
				Class.forName(FileUtils.getServerConfig().getConfigurationSection("server.softwares.types").getStringList(path).get(i));
				return true;
			}
