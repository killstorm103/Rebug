...
public class PT 
{
	...
	public static void sendChat (@NotNull Object object, String message, ChatType type)
	{
		...
		user.sendMessage(Component.text(message), type);
	}
	...
	public static void UpdateAntiCheat (User user, String[] ItemName, ItemStack item, CommandSender Sudo, boolean Multi) 
	{
		...
		if (Multi) 
		{
			...
			if (!FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats.multiple-anticheat.enabled")) 
			{
				if (Sudo == null) 
				{
			...
			final int maxACs = FileUtils.getLoadedAntiCheatsFile().getInt("loaded-anticheats.multiple-anticheat.max-anticheats");
			if (FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats.multiple-anticheat.limiter-enabled") && maxACs > 1 && size > maxACs) 
			{
				if (Sudo != null)
			...
			for (int i = 1; i < ItemName.length; i++) 
			{
				...
				if (!FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + AntiCheatName.toLowerCase() + ".enabled")) 
				{
					if (Sudo != null)
				...
				if (Sudo == null
						&& FileUtils.getLoadedAntiCheatsFile().get("loaded-anticheats." + AntiCheatName.toLowerCase() + ".permission-to-use") != null && FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + AntiCheatName.toLowerCase() + ".permission-to-use")
						&& !user.hasPermission(
				...
				if (FileUtils.getLoadedAntiCheatsFile().get("loaded-anticheats." + AntiCheatName.toLowerCase() + ".is-multi-enabled") == null || !FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + AntiCheatName.toLowerCase() + ".is-multi-enabled")) 
				{
					if (Sudo != null)
						Sudo.sendMessage(
				...
				if (FileUtils.getLoadedAntiCheatsFile().get("loaded-anticheats." + AntiCheatName.toLowerCase() + ".requires-reconnect") != null && FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + AntiCheatName.toLowerCase() + ".requires-reconnect"))
		...
		if (!ItemName[0].equalsIgnoreCase("Vanilla"))
		{
			...
			if (Sudo == null && FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + ItemName[0].toLowerCase() + ".requires-combining"))
			{
				if (!FileUtils.getLoadedAntiCheatsFile().getBoolean("bypass-requires-combining-permission") || !user.hasPermission("me.killstorm103.rebug.user.bypass_anticheatcombine") && !hasAdminPerms(user))
				{
		...
		user.setResetColorAC(true);
		if (user.getSelectedAntiCheats() > 0 && FileUtils.getLoadedAntiCheatsFile().get ("loaded-anticheats." + user.getAntiCheat().toLowerCase() + ".api") != null &&
		FileUtils.getLoadedAntiCheatsFile().getBoolean ("loaded-anticheats." + user.getAntiCheat().toLowerCase() + ".api"))
		{
			try
			{
		...
		if (FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats." + user.getAntiCheat().toLowerCase() + ".requires-reconnect"))
