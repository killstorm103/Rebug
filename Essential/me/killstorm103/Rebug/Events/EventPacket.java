...
public class EventPacket implements PacketListener
{
	...
	public void onPacketSend(PacketSendEvent e) 
	{
		...
		if (e.getPacketType() == PacketType.Play.Server.ENTITY_EFFECT) 
		{
			...
			if (effect.equalsIgnoreCase("resistance") && user.getSettingsBooleans().getOrDefault("Damage Resistance", false) || effect.equalsIgnoreCase("fire_resistance") && user.getSettingsBooleans().getOrDefault("Fire Resistance", false))
				return;
			...
			for (PotionEffect effectType : user.getPlayer().getActivePotionEffects()) 
			{
				...
				{
					if (effectType.getType().getName().equalsIgnoreCase("DAMAGE_RESISTANCE") && user.getSettingsBooleans().getOrDefault("Damage Resistance", false) || effectType.getType().getName().equalsIgnoreCase("FIRE_RESISTANCE") && user.getSettingsBooleans().getOrDefault("Fire Resistance", false)) {}
		...
		if (e.getPacketType() == PacketType.Play.Server.PLAYER_POSITION_AND_LOOK) 
		{
			...
			{
				final boolean isHSV = PT.isServerNewerOrEquals(ServerVersion.V_1_19);
		...
		if (e.getPacketType() == PacketType.Play.Server.SYSTEM_CHAT_MESSAGE)
		{
			...
			try
			{
				...
				if (RebugPlugin.End_Game_AntiCheats.isEmpty() && FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats.create-inventory")) 
				{
				...
				{
					if (FileUtils.getLoadedAntiCheatsFile().getBoolean("loaded-anticheats.create-inventory"))
