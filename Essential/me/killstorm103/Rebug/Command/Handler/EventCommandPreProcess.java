...
public class EventCommandPreProcess implements Listener
{
	...
	public void onPreCommand (PlayerCommandPreprocessEvent e)
	{
		...
		if (!RebugPlugin.getINSTANCE().getConfig().getBoolean("command.settings.allow-sub-commands", false)) return;
		...
		if (!RebugPlugin.getINSTANCE().getConfig().getBoolean("command.commands." + command.getName().toLowerCase() + ".shortcut-enabled", false)) 
		{
			RebugPlugin.Debug(player, command.getName() + "wasn't found or was Disabled in the config: commands: " + command.getName());
			return;
		}
		Bukkit.dispatchCommand(e.getPlayer(), RebugPlugin.getINSTANCE().getConfig().getString("command.settings.main-command.label", "rebug") + " " + command.getName() + (command.RemoveSlash() ? " " : "") + (command.RemoveSlash() ? message.replace("/", "") : message.replace(split[0], "")));
		e.setCancelled(true);
	}
}
