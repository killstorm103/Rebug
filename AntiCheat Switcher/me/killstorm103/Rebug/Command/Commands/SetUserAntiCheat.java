...
public class SetUserAntiCheat extends Command
{
	...
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".usage", "setuseranticheat <player> <ac>");
	}
	@Override
	public @NotNull String getDescription () 
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".description", "Manually set the AntiCheat of a user!");
	}
	@Override
	public @NotNull String getPermission()
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".permission", StartOfPermission + "set-user-anticheat");
	}
	@Override
	...
	public void onCommand(CommandSender sender, String command, String[] args) throws Exception {
		if (sender instanceof Player) 
		{
			...
			if (!PT.hasPermission(p, getPermission())) {
				p.sendMessage(RebugsAntiCheatSwitcherPlugin.getRebugMessage() + "You don't have Permission to do that!");
				return;
			}
		}
		if (args.length < 3) {
		...
		if (user == null) 
		{
			sender.sendMessage(RebugsAntiCheatSwitcherPlugin.getRebugMessage() + "Unknown User (User was null)!");
			return;
		}
