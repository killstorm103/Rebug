...
public class AntiCheatMenuCMD extends Command
{
	...
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".usage", "ac | ac <anticheat> | ac (anticheat) (anticheat) (...)");
	}
	@Override
	public @NotNull String getDescription () 
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".description", "opens a menu to select a AntiCheat!");
	}
	@Override
	public @NotNull String getPermission()
	{
		return RebugsAntiCheatSwitcherPlugin.getINSTANCE().getConfig().getString("command.commands." + getName().toLowerCase() + ".permission", StartOfPermission + "anticheat_select");
	}
	@Override
	...
	public List<String> onTabComplete(CommandSender sender, org.bukkit.command.Command command, String[] args, String alias)
	{
		...
		if (alias.equalsIgnoreCase(getName()) || rebug)
		{
			...
			if (alias.equalsIgnoreCase(getName()) && args.length >= 2 || rebug && args.length >= 3)
			{
				...
				list2.remove(0);
				return StringUtil.copyPartialMatches(args[args.length - 1], list2, list);
			}
			list2.addAll(ItemsAndMenusUtils.TabAntiCheats);
			return StringUtil.copyPartialMatches(args[args.length - 1], list2, list);
		}
