...
public class GrimAC extends ApiProvider
{
	...
	public void onHandle (Player player, String AntiCheat) throws Exception 
	{
		...
		if (GrimApi == null)
		{
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, AntiCheat + "'s Api was Null!");
			return;
		}
		...
		player.sendMessage(RebugsAntiCheatSwitcherPlugin.getRebugMessage() + (GrimApi.getAlertManager().hasVerboseEnabled(player) ? ChatColor.GREEN + "Enabled" : ChatColor.DARK_RED + "Disabled") + ChatColor.BOLD.toString() + ChatColor.DARK_GRAY + " Verbose Alerts for " + AntiCheat);
	}
}
