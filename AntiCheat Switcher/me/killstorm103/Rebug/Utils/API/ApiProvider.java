...
public abstract class ApiProvider 
{
	...
	public static void onApi (Player player, String AntiCheat, int AmountOfAntiCheats, int Normal_Is_Minus_1)
	{
		try
		{
			...
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "isPlayerNull: " + (player == null) + " isAntiCheatNull: " + (PT.isStringNull(AntiCheat)));
				return;
			}
			...
			if (FileUtils.getLoadedAntiCheatsFile().getBoolean ("loaded-anticheats." + AntiCheat.toLowerCase() + ".api.check.for-plugin", true))
			{
				...
				{
					RebugsAntiCheatSwitcherPlugin.Log(Level.WARNING, "AntiCheat: " + AntiCheat + " was not found or was Disabled when trying to invoke it's Api!");
					return;
				}
			}
			...
			if (clazz != null)
			{
				try
				{
					ReflectionUtils.getMethod(true, clazz, new String[] {"onHandle"}, true, new Class<?>[] {Player.class, String.class}).invoke(clazz.newInstance(), new Object[] {player, AntiCheat});
				}
				catch (Exception e) 
				{
			...
			else
			{
				if (RebugsAntiCheatSwitcherPlugin.getSettingsBooleans().getOrDefault("Debug", false))
				{
					RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "This AntiCheat (" + AntiCheat + ") does not have a Api or it's Api isn't supported!");
					RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "You should disable it's Api in the Loaded AntiCheats.yml config!");
				}
			}
		}
		catch (Exception e) 
