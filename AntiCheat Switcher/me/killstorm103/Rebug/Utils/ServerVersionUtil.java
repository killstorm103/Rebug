...
public class ServerVersionUtil 
{
	...
	@Getter @Setter
	private static boolean isBedRockSupported, isServerNewerThan1_8 = true, isServerNewerOrEquals1_19 = false, isSpoofedSoftWare = false, isFolia, isChatTypeGameInfoSupported = true;
	@Getter @Setter
	private static String Server_Version = "", VersionName = "", VersionOfSoftWare = "", SoftWareType = "", JavaVersion = "";
	@Getter @Setter
	...
	public static void CreateINSTANCE (String name)
	{
		...
		if (toString != null && toString.length() > 0)
		{
			...
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "While Force Scanning Rebug Detected the SoftWare Type" + (detected > 1 ? "s" : "") + ":");
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, toString);
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "But we're not setting it to that cause you have alert-only enabled in Server.yml!");
			}
			else
				name =  toString;
		...
		if (!isSpoofedSoftWare() && FileUtils.getServerConfig().get("server.softwares.types." + ServerVersionUtil.getSoftWareType()) != null && ServerVersionUtil.getDetectedSoftWares() == 1)
		{
			...
			{
				e.printStackTrace();
				RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "Failed to get the Server Software Version");
	...
	public static boolean isServerType (@NotNull String type) 
	{
		...
		{
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "type inside isServerType is @NotNull but is Null!");
			return false;
		}
