...
public class FileUtils 
{
	public static void createCustomConfig(String con)
	{
		...
		{
		case "server":
			ServerFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "Server.yml");
			if (!ServerFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				ServerFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("Server.yml", false);
			}
			ServerConfig = YamlConfiguration.loadConfiguration(ServerFile);
			break;

		case "config":
			ConfigFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "config.yml");
			if (!ConfigFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				ConfigFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("config.yml", false);
			}
			YamlConfiguration.loadConfiguration(ConfigFile);
			break;

		case "player settings":
			PlayerSettingsFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "Player Settings.yml");
			if (!PlayerSettingsFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				PlayerSettingsFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("Player Settings.yml", false);
			}
		...
		case "loaded items":
			LoadedItemsConfigFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "Items.yml");
			if (!LoadedItemsConfigFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				LoadedItemsConfigFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("Items.yml", false);
			}
			ItemsConfig = YamlConfiguration.loadConfiguration(LoadedItemsConfigFile);
			break;
			
		case "menus":
			MenusConfigFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "Menus.yml");
			if (!MenusConfigFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				MenusConfigFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("Menus.yml", false);
			}
			MenusConfig = YamlConfiguration.loadConfiguration(MenusConfigFile);
			break;

		case "loaded anticheats":
			LoadedAntiCheatsConfigFile = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "Loaded AntiCheats.yml");
			if (!LoadedAntiCheatsConfigFile.exists()) 
			{
				RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Generating " + con + ".yml file!");
				LoadedAntiCheatsConfigFile.getParentFile().mkdirs();
				RebugsAntiCheatSwitcherPlugin.getINSTANCE().saveResource("Loaded AntiCheats.yml", false);
			}
			LoadedAntiCheatsFile = YamlConfiguration.loadConfiguration(LoadedAntiCheatsConfigFile);
			break;

		default:
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "Unknown Config File!");
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "FileUtils:createCustomConfig");
			break;
		}
	}
	public static void savePlayer(@NotNull Object object) 
	{
		...
		if (player == null)
		{
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "Player in FileUtils:savePlayer was Somehow Null!");
			return;
		}
		...
		playerConfig.getConfigurationSection("Player Settings").set("Client Command Prefix", PT.isStringNull(user.ClientCommandPreFix) ? "." : user.ClientCommandPreFix);
	...
	public static void restorePlayer (@NotNull Object object)
	{
		...
		if (player == null)
		{
			RebugsAntiCheatSwitcherPlugin.Log(Level.SEVERE, "Player in FileUtils:restorePlayer was Somehow Null!");
			return;
		}
		...
		final String def = getLoadedAntiCheatsFile().getString("default-anticheat.anticheat", "Vanilla");
		// TODO Add AntiCheat Apis here
		if (user.getSelectedAntiCheats() < 2) 
		{
			user.setAntiCheat(PT.hasAdminPerms(user) || getLoadedAntiCheatsFile().getBoolean("default-anticheat.bypass-permission-enabled", false) && user.getPlayer().hasPermission(getLoadedAntiCheatsFile().getString("default-anticheat.permission", "me.killstorm103.rebug.user.bypass_force_default_anticheat"))
		...
		else
		{
			...
			final String[] check = PT.SplitString(user.getAntiCheat());
		...
		user.ClientCommandPreFix = playerConfig.getConfigurationSection("Player Settings").getString("Client Command Prefix", ".");
	}
	public static void initFolder()
	{
		folder = new File(RebugsAntiCheatSwitcherPlugin.getINSTANCE().getDataFolder(), "player-data");
		if (!folder.exists())
		{
			RebugsAntiCheatSwitcherPlugin.Log(Level.INFO, "Creating player-data folder!");
			folder.mkdirs();
		}
	}
